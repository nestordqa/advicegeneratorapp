...
const { Videogame, Genres} = require('../db.js') //Requiero Models y la conexion (Por si se llega a necesitar)
...
router.post('/allVideogames',async(req,res)=>{
    ...
    const allAtAll = [...getAll, ...videogameDB];
     ...
     for(let i = 0; i < allAtAll.length; i++){
        ...
        if(Array.isArray(allAtAll[i].Generos)){ //Verifico si el genero es un arreglo, si lo es, entra en la ejecucion de esta condicion
            for(let j = 0; allAtAll[i].Generos.length > j; j++){
                let genresDB = await Genres.findAll({where:{Nombre: allAtAll[i].Generos[j]}});//Evaluo genero por genero para comprobar si ya està en la DB
                if(genresDB.length == 0){
                    await newGame.createGenre({Nombre: allAtAll[i].Generos[j], Created: true});// Se 
                }
                await newGame.setGenres(genresDB);
            }
            }else{
            let genresDB = await Genres.findAll({where:{Nombre: allAtAll[i].Generos}}); 
            if(genresDB.length == 0){
                await newGame.createGenre({Nombre: allAtAll[i].Generos, Created: true});
            };            
            await newGame.setGenres(genresDB);
...
router.post('/videogames', async(req,res)=>{
    const {Nombre, Descripcion, Fecha_lanzamiento, Rating, Plataformas, Img_URL, genre} = req.body; //Traigo todos datos a cargar en DB
    try{
        if(!Nombre || !Descripcion || !Plataformas || !Img_URL || !genre){ // Chequeo que no falten aquellos que son imprescindibles
            res.status(400).json({Error: "Faltan datos"})
        }else{          
            var newGame= await Videogame.create({Nombre, Descripcion, Fecha_lanzamiento, Rating, Plataformas, Img_URL, Created: true}); //Se sube nuevo videogame a DB
            if(Array.isArray(genre)){ //Verifico si el genero es un arreglo, si lo es, entra en la ejecucion de esta condicion
                for(let j = 0; genre.length > j; j++){
                    let genresDB = await Genres.findAll({where:{Nombre: genre[j]}});//Evaluo genero por genero para comprobar si ya està en la DB
                    await newGame.setGenres(genresDB);
                }}
